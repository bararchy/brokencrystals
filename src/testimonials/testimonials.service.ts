...
export class TestimonialsService {
  ...
  async count(query: string): Promise<number> {
    try {
      ...
      // Validate the query to ensure it is a count query
      if (!/^select\s+count\(\*\)\s+as\s+count\s+from\s+testimonial;?$/i.test(query.trim())) {
        throw new BadRequestException('Invalid query format. Only count queries are allowed.');
      }
