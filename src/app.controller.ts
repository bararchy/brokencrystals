...
export class AppController {
  ...
  async renderTemplate(@Body() raw): Promise<string> {
    if (typeof raw === 'string' || Buffer.isBuffer(raw)) {
      ...
      const allowedVariables = ['name', 'age'];
      const templateData = { name: 'User', age: 30 }; // Example data
      ...
      if (disallowedVariables.length > 0) {
        throw new HttpException('Disallowed template variables', HttpStatus.BAD_REQUEST);
      }
