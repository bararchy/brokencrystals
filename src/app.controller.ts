...
export class AppController {
  ...
  async renderTemplate(@Body() raw): Promise<string> {
    if (typeof raw === 'string' || Buffer.isBuffer(raw)) {
      ...
      // Use a predefined template to avoid direct user input compilation
      const template = 'Hello, {{=it.name}}!';
      const compiled = dotT.template(template);
      const data = { name: text }; // Assume the input is a name
  ...
  getSecrets(): Record<string, string> {
    ...
    // Filter out any empty secrets to avoid exposing them
    return Object.fromEntries(Object.entries(secrets).filter(([_, value]) => value !== ''));
  }
