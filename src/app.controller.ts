...
export class AppController {
  ...
  async renderTemplate(@Body() raw): Promise<string> {
    if (typeof raw === 'string' || Buffer.isBuffer(raw)) {
      ...
      // Implementing a basic allowlist approach
      const allowedTemplates = ['template1', 'template2']; // Example allowed templates
      if (!allowedTemplates.includes(text)) {
        throw new HttpException('Template not allowed', HttpStatus.FORBIDDEN);
      }
  ...
  getSecrets(): Record<string, string> {
    // Removed hardcoded secrets and replaced with environment variables
    const secrets = {
